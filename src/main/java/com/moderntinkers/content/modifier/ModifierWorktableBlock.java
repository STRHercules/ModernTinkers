...
/** Server-authoritative tool modifier application block. */
public final class ModifierWorktableBlock extends BaseEntityBlock {
    ...
    @Override
    public MapCodec<ModifierWorktableBlock> codec() {
        return CODEC;
    }
    ...
    protected MenuProvider getMenuProvider(BlockState state, Level level,
        ...
        return blockEntity instanceof ModifierWorktableBlockEntity worktable ? worktable : null;
    }
    ...
    public void onRemove(BlockState state, Level level, net.minecraft.core.BlockPos pos,
        ...
        if (!state.is(newState.getBlock())) {
            BlockEntity blockEntity = level.getBlockEntity(pos);
            if (blockEntity instanceof ModifierWorktableBlockEntity worktable) {
                Containers.dropContents(level, pos, worktable.getInputInventory());
            }
        }
